...
4. 首次開啟 App 若已設定 Client ID 且瀏覽器曾授權過，會嘗試**靜默**上傳一次備份；登入狀態不持久（關閉分頁後 Token 失效屬正常），需再登入才能手動備份／還原。
...
- **上次雲端備份時間**存在 **`fl_last_gdrive_backup`**（localStorage，ISO 時間字串）。
- 安裝為 PWA 後資料較不受 Safari 7 天清除機制影響。
- 設定內「本機儲存用量」優先使用 **`navigator.storage.estimate()`**（整站配額估算），不再僅限約 5MB 的 localStorage；若瀏覽器不支援則退回約略計算。快滿時仍請匯出備份並刪除舊紀錄。
