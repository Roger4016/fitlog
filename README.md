...
> 使用 **Gemini 2.5 Flash**；免費額度依 [Google 官方](https://ai.google.dev/pricing) 與 AI Studio 的 Rate limit 為準。

### 2. 設定 API Key

1. 開啟 FitLog 網頁
2. 首頁會顯示琥珀色橫幅「尚未設定 Gemini API Key」，點擊 **設定**
3. 在設定頁的「Gemini API Key」欄位貼上剛複製的 Key
4. 點擊 **儲存 API Key**

> 沒有 Key 也能使用，只是 AI 分析功能不可用，仍可手動輸入所有紀錄。
...
- API Key 存在 `fl_gemini_key`，不會上傳到任何伺服器
- 安裝為 PWA 後資料不受 Safari 7 天清除機制影響
- 容量約 5-10MB，足夠記錄數年的飲食運動資料

## API Key 安全性

- Key 只存在你的裝置上，不會隨程式碼一起上傳
