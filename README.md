...
4. **Google 登入／Token**：OAuth access token **不寫入**本機（僅存在記憶體）；殺掉 PWA 或關閉分頁後會清空。重開後若瀏覽器仍保留對 Google 的授權，會先以 **`prompt` 空白** 嘗試**靜默**取回 token（首頁載入時會靜默同步「上次備份」時間並嘗試上傳備份；開啟設定時也會再試一次以更新「已登入」按鈕狀態）。若靜默失敗（Cookie／ITP／未授權過等），再手動按「登入 Google」即可。
...
- **上次雲端備份時間**存在 **`fl_last_gdrive_backup`**（localStorage，ISO 時間字串）；他裝置備份後，本機顯示會在**已登入 Google** 且**開啟設定**（或執行還原）時，向 Drive 對齊備份檔的修改時間。
